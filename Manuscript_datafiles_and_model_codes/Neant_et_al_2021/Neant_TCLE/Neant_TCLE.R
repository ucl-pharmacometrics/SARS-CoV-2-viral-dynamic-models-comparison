...
mod.dir <- "/home/akosua/Documents/Model_evaluation/SARS_CoV-2_viral_dynamic_models_comparison/Neant_et_al_2021/Neant_TCLE/"
setwd(mod.dir)
library(ggplot2)
library(scales)
library(deSolve)
...
TCLE <- function() {
  ini({
    ...
    eta.beta +  eta.delta + eta.rho + eta.c + eta.k ~ c(1,
                                                        0.01, 1,
                                                        0.01, 0.01, 1,
                                                        0.01, 0.01, 0.01, 1,
...
dev.off()
#
run.rec[1, ] <- c("0.5", "vl_dat_14days_post_symptom_under65", "TCLE model", round(run0.5.lst$OBJF),round(run0.5.lst$AIC), round(run0.5.lst$BIC) )
#
#################
#
##### read data ######################################################################## 1 Day
...
dev.off()
#
run.rec[2, ] <- c("1", "vl_dat_14days_post_symptom_under65", "TCLE model", round(run1.lst$OBJF),round(run1.lst$AIC), round(run1.lst$BIC) )
#
#################
#
##### read data ######################################################################## 1.5 Days
...
dev.off()
#
run.rec[3, ] <- c("1.5", "vl_dat_14days_post_symptom_under65", "TCLE model", round(run1.5.lst$OBJF),round(run1.5.lst$AIC), round(run1.5.lst$BIC) )
#
#################
#
##### read data ######################################################################## 2 Days
...
dev.off()
#
run.rec[4, ] <- c("2", "vl_dat_14days_post_symptom_under65", "TCLE model", round(run2.lst$OBJF),round(run2.lst$AIC), round(run2.lst$BIC) )
#
#################
#
##### read data ######################################################################## 3 Days
...
dev.off()
#
run.rec[5, ] <- c("3", "vl_dat_14days_post_symptom_under65", "TCLE model", round(run3.lst$OBJF),round(run3.lst$AIC), round(run3.lst$BIC) )
#
#################
#
##### read data ######################################################################## 4 Days
...
dev.off()
#
run.rec[6, ] <- c("4", "vl_dat_14days_post_symptom_under65", "TCLE model", round(run4.lst$OBJF),round(run4.lst$AIC), round(run4.lst$BIC) )
#
#################
#
##### read data ######################################################################## 5 Days
...
dev.off()
#
run.rec[7, ] <- c("5", "vl_dat_14days_post_symptom_under65", "TCLE model", round(run5.lst$OBJF),round(run5.lst$AIC), round(run5.lst$BIC) )
#
#################
#
##### read data ######################################################################## 6 Days
...
dev.off()
#
run.rec[8, ] <- c("6", "vl_dat_14days_post_symptom_under65", "TCLE model", round(run6.lst$OBJF),round(run6.lst$AIC), round(run6.lst$BIC) )
#
#################
#
##### read data ######################################################################## 7 Days
...
dev.off()
#
run.rec[9, ] <- c("7", "vl_dat_14days_post_symptom_under65", "TCLE model", round(run7.lst$OBJF),round(run7.lst$AIC), round(run7.lst$BIC) )
#
#################
#
##### read data ######################################################################## 8 Days
...
dev.off()
#
run.rec[10, ] <- c("8", "vl_dat_14days_post_symptom_under65", "TCLE model", round(run8.lst$OBJF),round(run8.lst$AIC), round(run8.lst$BIC) )
#
#################
#
##### read data ######################################################################## 9 Days
...
dev.off()
#
run.rec[11, ] <- c("9", "vl_dat_14days_post_symptom_under65", "TCLE model", round(run9.lst$OBJF),round(run9.lst$AIC), round(run9.lst$BIC) )
#
#################
#
##### read data ######################################################################## 10 Days
...
dev.off()
#
run.rec[12, ] <- c("10", "vl_dat_14days_post_symptom_under65", "TCLE model", round(run10.lst$OBJF),round(run10.lst$AIC), round(run10.lst$BIC) )
#
###########################
#
##### read data ######################################################################## 11 Days
...
dev.off()
#
run.rec[13, ] <- c("11", "vl_dat_14days_post_symptom_under65", "TCLE model", round(run11.lst$OBJF),round(run11.lst$AIC), round(run11.lst$BIC) )
#
###########################
#
##### read data ######################################################################## 12 Days
...
dev.off()
#
run.rec[14, ] <- c("12", "vl_dat_14days_post_symptom_under65", "TCLE model", round(run12.lst$OBJF),round(run12.lst$AIC), round(run12.lst$BIC) )
#
###########################
#
##### read data ######################################################################## 13 Days
...
dev.off()
#
run.rec[15, ] <- c("13", "vl_dat_14days_post_symptom_under65", "TCLE model", round(run13.lst$OBJF),round(run13.lst$AIC), round(run13.lst$BIC) )
#
##########################
#
##### read data ######################################################################## 14 Days
...
plot1 <- vpc1 + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
  ...
  labs(x = "", y = "DV",
       title = paste("VPC")) + 
  scale_x_continuous(limits = c(13, 30), breaks = seq(0,30,4)) +
  scale_y_continuous(limits = c(-10, 30), breaks = seq(0,30,10)) 
plot2 <- vpc2 + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                                   panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) + 
  theme(plot.title = element_text(hjust = 0.5)) + theme (axis.title.y = element_text(size = 10))+
...
dev.off()
#
run.rec[16, ] <- c("14", "vl_dat_14days_post_symptom_under65", "TCLE model", round(run14.lst$OBJF),round(run14.lst$AIC), round(run14.lst$BIC) )
#
...
c <- 2.62
#
R0 <- (rho*beta*A_T)/(delta*(c + beta*A_T)) # 2907.61
#
...
#
## Fit the model 
run1.5.lstRo <- nlmixr(TCLERo, vl_dat, est = "saem",
                       control = saemControl(
                         seed = 99,
                         nu = c(2,2,2)),
                       table = tableControl(npde = TRUE, censMethod = c("cdf")))
...
c <- 0.94  
#
R0 <- (rho*beta*A_T)/(delta*(c + beta*A_T)) # c_fixed (10) = 2332.30  , c/b_fixed (10/0.000829) = 88960.81 , rho_fixed (10) = 24.09 
#
### Duration of virus production (L)
L <- 1/delta # c_fixed (10) = 1.51 , c/b_fixed (10/0.000829) = 1.21 , rho_fixed (10) = 2.44
#
## save output to use later for rho_fixed = 10
saveRDS(run1.5.lstRo, "run1.5.lstRo")
run1.5.lstRo <- readRDS("run1.5.lstRo")
## OBF, AIC , BIC for rho_fixed = 10
run1.5.lstRo$OBJF # 1777.834
run1.5.lstRo$AIC # 3432.517
run1.5.lstRo$BIC # 3504.284
#
###### end ########
