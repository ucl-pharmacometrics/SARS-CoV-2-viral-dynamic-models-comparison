...
mod.dir <- "/home/akosua/Documents/Model_evaluation/SARS_CoV-2_viral_dynamic_models_comparison/Neant_et_al_2021/Neant_TCL/"
setwd(mod.dir)
library(ggplot2)
library(scales)
library(deSolve)
...
basic.gof <- function(run.dat){
  ...
  npde.tad <- ggplot() + 
    ...
    geom_smooth(aes(x = TIME, y = NPDE), 
                data = run.dat, span = 5/3, se = FALSE, col = "red", lwd = 1) +
    scale_x_continuous(limits = c(4, 20), breaks = seq(0,20,4))
  #
  npde.pred <- ggplot() + 
    theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                       panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) +
...
TCL <- function() {
  ini({
    tbeta  <- -7.37        # Rate constant for virus infection
    tdelta <- 0.9          # Death rate of infected cells
    trho   <- -7.83        # Viral replication
    tc     <-  1.10       # Viral clearance  
    
    ### initial estimates are ln values of beta = 10^(-3.2), delta = 2.5, rho = 10^(-3.4), c = 3)
    
    eta.beta + eta.delta + eta.rho + eta.c ~ c(1,
                                               0.01, 1,
                                               0.01, 0.01, 1,
                                               0.01, 0.01, 0.01, 1)
  ...
  model({
    ...
    d/dt(A_T) <- -beta * A_T * A_v
    d/dt(A_I)  <- beta * A_T * A_v - delta * A_I
    d/dt(A_v)  <-  rho * A_I - c * A_v
...
#
## Fit the model 
run0.5.lst <- nlmixr(TCL, vl_dat, est = "saem",
                     control = saemControl(
                     seed = 99,
                    nu = c(2,2,2)),
...
dev.off()
#
run.rec[1, ] <- c("0.5", "vl_dat_14days_post_symptom_under65", "TCL model", round(run0.5.lst$OBJF),round(run0.5.lst$AIC), round(run0.5.lst$BIC) )
#
#################
#
##### read data ######################################################################## 1 Day
...
#
## Fit the model 
run1.lst <- nlmixr(TCL, vl_dat, est = "saem",
                   control = saemControl(
                   seed = 99,
                   nu = c(2,2,2)),
                   table = tableControl(npde = TRUE, censMethod = c("cdf")))
...
dev.off()
#
run.rec[2, ] <- c("1", "vl_dat_14days_post_symptom_under65", "TCL model", round(run1.lst$OBJF),round(run1.lst$AIC), round(run1.lst$BIC) )
#
#################
#
##### read data ######################################################################## 1.5 Days
...
#
## Fit the model 
run1.5.lst <- nlmixr(TCL, vl_dat, est = "saem",
                     control = saemControl(
                       seed = 99,
                       nu = c(2,2,2)),
                     table = tableControl(npde = TRUE, censMethod = c("cdf")))
...
dev.off()
#
run.rec[3, ] <- c("1.5", "vl_dat_14days_post_symptom_under65", "TCL model", round(run1.5.lst$OBJF),round(run1.5.lst$AIC), round(run1.5.lst$BIC) )
#
#################
#
##### read data ######################################################################## 2 Days
...
#
## Fit the model 
run2.lst <- nlmixr(TCL, vl_dat, est = "saem",
                   control = saemControl(
                     seed = 99,
                     nu = c(2,2,2)),
                   table = tableControl(npde = TRUE, censMethod = c("cdf")))
...
dev.off()
#
run.rec[4, ] <- c("2", "vl_dat_14days_post_symptom_under65", "TCL model", round(run2.lst$OBJF),round(run2.lst$AIC), round(run2.lst$BIC) )
#
#################
#
##### read data ######################################################################## 3 Days
...
#
## Fit the model 
run3.lst <- nlmixr(TCL, vl_dat, est = "saem",
                   control = saemControl(
                     seed = 99,
                     nu = c(2,2,2)),
                   table = tableControl(npde = TRUE, censMethod = c("cdf")))
...
dev.off()
#
run.rec[5, ] <- c("3", "vl_dat_14days_post_symptom_under65", "TCL model", round(run3.lst$OBJF),round(run3.lst$AIC), round(run3.lst$BIC) )
#
#################
#
##### read data ######################################################################## 4 Days
...
#
## Fit the model 
run4.lst <- nlmixr(TCL, vl_dat, est = "saem",
                   control = saemControl(
                     seed = 99,
                     nu = c(2,2,2)),
                   table = tableControl(npde = TRUE, censMethod = c("cdf")))
...
dev.off()
#
run.rec[6, ] <- c("4", "vl_dat_14days_post_symptom_under65", "TCL model", round(run4.lst$OBJF),round(run4.lst$AIC), round(run4.lst$BIC) )
#
#################
#
##### read data ######################################################################## 5 Days
...
#
## Fit the model 
run5.lst <- nlmixr(TCL, vl_dat, est = "saem",
                   control = saemControl(
                     seed = 99,
                     nu = c(2,2,2)),
                   table = tableControl(npde = TRUE, censMethod = c("cdf")))
...
dev.off()
#
run.rec[7, ] <- c("5", "vl_dat_14days_post_symptom_under65", "TCL model", round(run5.lst$OBJF),round(run5.lst$AIC), round(run5.lst$BIC) )
#
#################
#
##### read data ######################################################################## 6 Days
...
#
## Fit the model 
run6.lst <- nlmixr(TCL, vl_dat, est = "saem",
                   control = saemControl(
                     seed = 99,
                     nu = c(2,2,2)),
                   table = tableControl(npde = TRUE, censMethod = c("cdf")))
...
dev.off()
#
run.rec[8, ] <- c("6", "vl_dat_14days_post_symptom_under65", "TCL model", round(run6.lst$OBJF),round(run6.lst$AIC), round(run6.lst$BIC) )
#
#################
#
##### read data ######################################################################## 7 Days
...
#
## Fit the model 
run7.lst <- nlmixr(TCL, vl_dat, est = "saem",
                   control = saemControl(
                     seed = 99,
                     nu = c(2,2,2)),
                   table = tableControl(npde = TRUE, censMethod = c("cdf")))
...
dev.off()
#
run.rec[9, ] <- c("7", "vl_dat_14days_post_symptom_under65", "TCL model", round(run7.lst$OBJF),round(run7.lst$AIC), round(run7.lst$BIC) )
#
#################
#
##### read data ######################################################################## 8 Days
...
#
## Fit the model 
run8.lst <- nlmixr(TCL, vl_dat, est = "saem",
                   control = saemControl(
                     seed = 99,
                     nu = c(2,2,2)),
                   table = tableControl(npde = TRUE, censMethod = c("cdf")))
...
dev.off()
#
run.rec[10, ] <- c("8", "vl_dat_14days_post_symptom_under65", "TCL model", round(run8.lst$OBJF),round(run8.lst$AIC), round(run8.lst$BIC) )
#
#################
#
##### read data ######################################################################## 9 Days
...
#
## Fit the model 
run9.lst <- nlmixr(TCL, vl_dat, est = "saem",
                   control = saemControl(
                     seed = 99,
                     nu = c(2,2,2)),
                   table = tableControl(npde = TRUE, censMethod = c("cdf")))
...
dev.off()
#
run.rec[11, ] <- c("9", "vl_dat_14days_post_symptom_under65", "TCL model", round(run9.lst$OBJF),round(run9.lst$AIC), round(run9.lst$BIC) )
#
#################
#
##### read data ######################################################################## 10 Days
...
#
## Fit the model 
run10.lst <- nlmixr(TCL, vl_dat, est = "saem",
                    control = saemControl(
                      seed = 99,
                      nu = c(2,2,2)),
                    table = tableControl(npde = TRUE, censMethod = c("cdf")))
...
dev.off()
#
run.rec[12, ] <- c("10", "vl_dat_14days_post_symptom_under65", "TCL model", round(run10.lst$OBJF),round(run10.lst$AIC), round(run10.lst$BIC) )
#
###########################
#
##### read data ######################################################################## 11 Days
...
#
## Fit the model 
run11.lst <- nlmixr(TCL, vl_dat, est = "saem",
                    control = saemControl(
                      seed = 99,
                      nu = c(2,2,2)),
                    table = tableControl(npde = TRUE, censMethod = c("cdf")))
...
dev.off()
#
run.rec[13, ] <- c("11", "vl_dat_14days_post_symptom_under65", "TCL model", round(run11.lst$OBJF),round(run11.lst$AIC), round(run11.lst$BIC) )
#
###########################
#
##### read data ######################################################################## 12 Days
...
#
## Fit the model 
run12.lst <- nlmixr(TCL, vl_dat, est = "saem",
                    control = saemControl(
                      seed = 99,
                      nu = c(2,2,2)),
                    table = tableControl(npde = TRUE, censMethod = c("cdf")))
...
dev.off()
#
run.rec[14, ] <- c("12", "vl_dat_14days_post_symptom_under65", "TCL model", round(run12.lst$OBJF),round(run12.lst$AIC), round(run12.lst$BIC) )
#
###########################
#
##### read data ######################################################################## 13 Days
...
#
## Fit the model 
run13.lst <- nlmixr(TCL, vl_dat, est = "saem",
                    control = saemControl(
                      seed = 99,
                      nu = c(2,2,2)),
                    table = tableControl(npde = TRUE, censMethod = c("cdf")))
...
dev.off()
#
run.rec[15, ] <- c("13", "vl_dat_14days_post_symptom_under65", "TCL model", round(run13.lst$OBJF),round(run13.lst$AIC), round(run13.lst$BIC) )
#
##########################
#
##### read data ######################################################################## 14 Days
...
#
## Fit the model 
run14.lst <- nlmixr(TCL, vl_dat, est = "saem",
                    control = saemControl(
                      seed = 99,
                      nu = c(2,2,2)),
                    table = tableControl(npde = TRUE, censMethod = c("cdf")))
...
plot1 <- vpc1 + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
  ...
  labs(x = "", y = "DV",
       title = paste("VPC")) + 
  scale_x_continuous(limits = c(12, 30), breaks = seq(0,30,4)) +
  scale_y_continuous(limits = c(-10, 30), breaks = seq(0,30,10)) 
plot2 <- vpc2 + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
                                   panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) + 
  theme(plot.title = element_text(hjust = 0.5)) + theme (axis.title.y = element_text(size = 10))+
...
dev.off()
#
run.rec[16, ] <- c("14", "vl_dat_14days_post_symptom_under65", "TCL model", round(run14.lst$OBJF),round(run14.lst$AIC), round(run14.lst$BIC) )
#
...
c <- 0.93
#
R0 <- (rho*beta*A_T)/(delta*c) # 64893.93
#
...
#
## Fit the model 
run1.lstRo <- nlmixr(TCLRo, vl_dat, est = "saem",
                       control = saemControl(
                         seed = 99,
                         nu = c(2,2,2)),
                       table = tableControl(npde = TRUE, censMethod = c("cdf")))
...
c <- 5.34
#
R0 <- (rho*beta*A_T)/(delta*c) # c_fixed (10) =1224406.02 , c/b_fixed (10/0.000829) = 37937.288 , rho_fixed (10) = 126507.57
#
### Duration of virus production (L)
L <- 1/delta # c_fixed (10) = 1.50, c/b_fixed (10/0.000829) = 1.50 , rho_fixed (10) = 1.16
#
#### end ###
